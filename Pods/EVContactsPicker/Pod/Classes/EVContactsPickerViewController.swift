...
@objc public class EVContactsPickerViewController: UIViewController, UITableViewDataSource, UITableViewDelegate,  EVPickedContactsViewDelegate, CNContactViewControllerDelegate {
    ...
    func setup() -> Void {
        self.title  = NSBundle.evLocalizedStringForKey("Selected Friends") + "(0)"
    ...
    override public func viewDidLoad() {
        ...
        self.contactPickerView?.setPlaceHolderString(NSBundle.evLocalizedStringForKey("Type Friend Name"))
...
//                let im = UIImage(contentsOfFile: imPath!)
    ...
    public func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_BACKGROUND, 0)) {
            ...
            let URL:NSURL = NSURL(string: (contact?.imageString)!)!
            let data:NSData = NSData(contentsOfURL: URL)!
            let image:UIImage = UIImage(data: data)!
            //self.downloadedImages[self.friends![indexPath.row]] = image
...
//                let im = UIImage(contentsOfFile: imPath!)
        ...
        self.title = String(NSBundle.evLocalizedStringForKey("Add Friends") + "(\(self.selectedContacts!.count))")
        self.filteredContacts = self.contacts
        self.tableView?.reloadData()
